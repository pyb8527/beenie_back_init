...
support:*         ─→ (다른 모듈에 의존하지 않음)
```

**규칙**

1. `support:*`는 어떤 모듈에도 의존하지 않는다. (최하위 라이브러리)
...
- **의존 방향 강제**: 코드 리뷰만으로 규칙을 지키기 어려워지면 ArchUnit 테스트를 도입해
  "domain 패키지는 infrastructure/storage 패키지를 참조할 수 없다" 같은 규칙을 CI에서 검증한다.
